    ...
    setTimeout(function() {
        if (isNew) {
            $.each(locations, function(index, location) {
                $(parent).append(
                    '<div class="location" id="location" data-location="' + location.location + '" data-type="normal" data-label="' + location.label + '">' +
                        '<p><span id="' + location.location + '">' + location.label + '</span></p>' + // Menambahkan format baru
                    '</div>'
                );
            });
        } else {
